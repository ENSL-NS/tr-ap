...
- commits:
    - author: Pablo Carranza Velez
      ...
      subject: >-
        Fix a scoping issue that would cause the digest function to be
...
- commits:
    - author: Pablo Carranza Velez
      body: >-
        ...
        when the image has digest instead of tag. Otherwise behavior us the same
        as before.

        compileRegistryAndName now accepts a digest field too.

        normaliseImageName worked with digests but it was just lucky, as
        getRegistryAndName would match the "@sha256" as part of the repo.

        This change makes the support intentional, and matching the image name
        in the correct way.
      footers:
        change-type: minor
        signed-off-by: Pablo Carranza Velez <pablo@resin.io>
      ...
      subject: >-
        Allow getRegistryAndName, compileRegistryAndName, and normaliseImageName
        to work with images with digest
...
- commits:
    ...
    - author: Akis Kesoglou
      body: >-
        ...
        For example the diff directory
        `/var/lib/docker/overlay2/223c2864175491657d238e2664251df13b63adb8d050924fd1bfcdb278b866f7/diff`
        now is referenced as `l/NFYKDW6APBCCUCTOUSYDH4DXAT`, which is much
        shorter.

        See:
        https://docs.docker.com/engine/userguide/storagedriver/overlayfs-driver/#image-and-container-layers-on-disk
      footers:
        change-type: patch
...
- commits:
    - author: Pablo Carranza Velez
      body: >-
        ...
        Now we extend that class and use dockerode's promise support instead of
        promisifying - but we pass bluebird

        as the Promise for dockerode to use.

        We also add mocha (with mochainon) and some basic tests that only cover
        testing that this returns a valid promise-aware

        docker object.

        No tests for the docker-toolbelt specific functions were added yet
      ...
      subject: >-
        Use dockerode's promise functionality (docker will not have
...
- commits:
    ...
    - author: Akis Kesoglou
      body: >-
        ...
        This deprecates `aufsDiffPaths`, which now also applies to overlay2,
        introducing a new method `diffPaths` that can work with both aufs and
        overlay2.
      footers:
        change-type: minor
...
- commits:
    ...
    - author: Pablo Carranza Velez
      body: >-
        ...
        scheme. We add a `true` third argument to the semver comparison to make
        it

        more forgiving and properly compare 17.03.1-ce and the like as if they
        were semver.
      footers:
        change-type: patch
        signed-off-by: Pablo Carranza Velez <pablo@resin.io>
...
- commits:
    ...
    - author: Pablo Carranza Velez
      body: >-
        ...
        to mount when layers don't fit in that: we mount with as much as we can
        fit and then

        remount appending each remaining layer. We stick to 4096 instead of
        actually finding the page size

        cause it's assured to work (4096 is the minimum in the arches we
        support), reduces complexity and

        shouldn't generate much overhead (we don't care about a few extra
        remounts).

        Based on
        https://github.com/docker/docker/blob/b937aa8e6968d805527d163e6f477d496ceb88d7/daemon/graphdriver/aufs/aufs.go#L595

        We also introduce 8 random characters to the temporary path we use for
        the aufs mount, so that
      ...
      subject: >-
        Fix mounting aufs when there's several layers and use random path for
        mount.
...
- commits:
    ...
    - author: Pablo Carranza Velez
      body: >-
        ...
        To have fully compatible behavior, imageRootDirMounted should be used,
        so that

        the full image rootfs gets mounted at the returned path. For other
        drivers,

        behavior will be the same as imageRootDir (only it has to be used with
        Promise.using).

        aufsDiffPaths allows getting the paths for each layer diff in an
        aufs-stored image.
